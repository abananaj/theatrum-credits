...
function filterByRoleGroup(credits, roleGroup) {
	if (roleGroup === 'all' || !roleGroup) {
		return credits;
	}
	if (roleGroup === 'team') {
		return credits.filter(
			(c) => !TEAM_EXCLUDE_GROUPS.includes(c.role_group)
		);
	}
	if (roleGroup === 'cast') {
		return credits.filter((c) => c.role_group === 'actor');
	}
	if (roleGroup === 'partner') {
		return credits.filter((c) => c.role_group === 'producer');
	}
	return credits;
}
			...
			<li key={credit.id} className="credit">
				<div className="credit-link">
					{credit.artist_thumbnail && (
						<img
							src={credit.artist_thumbnail}
							alt={artistTitle}
							className="artist-headshot"
						/>
					)}
			...
			<InspectorControls>
				...
				<PanelBody title={__('Layout', 'theatrum-credits')}>
					...
					<TextControl
						...
						help={__(
							'Any CSS length, e.g. 160px, 10rem, 20%',
							'theatrum-credits'
						)}
						value={itemWidth}
						onChange={(value) =>
